...
function addThisPage($title = null, $act = null , $level = null)    // add the calling page
{
    ...
    $arr_links    = $this->controller->Session->read('crumb_links');
    $arr_titles    = $this->controller->Session->read('crumb_titles');
    $arr_levels    = $this->controller->Session->read('crumb_levels');

    if (is_null($level) AND $this->controller->params['action'] != 'index' && $level != 'auto') {
        $level = $controller ;
    }
    if ( $level == 'unique'){
 ...
 */
function getHtml($title = null, $action=null, $level = null, &$controllerref)
{
